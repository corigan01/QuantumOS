...
fn use_std_mutex(mutex: &std::sync::Mutex<i32>) {
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    for thread_count in [0, 2, 4, machine_thread_count] {
        group.bench_function(
            &format!(
                "Kinases: Mutex locking with {} contention threads",
                thread_count
            ),
            |b| {
    ...
    let mut group = c.benchmark_group("Kinases: Async Mutex Lock with threads");
    for thread_count in [0, 2, 4, machine_thread_count] {
        group.bench_function(
            &format!(
    ...
    let mut group = c.benchmark_group("Comparison between Kinases and std::sync");
    for thread_count in [0, 2, 4, machine_thread_count] {
        group.bench_function(
            &format!(
                "Kinases: Mutex locking with {} contention threads",
        ...
        group.bench_function(
            &format!(
                "std::sync: Mutex locking with {} contention threads",
                thread_count
            ),
            |b| {
